...
CREATE OR REPLACE TRIGGER infantry_combat_vehicles_update_trigger
    BEFORE UPDATE
    ON infantry_combat_vehicles
    FOR EACH ROW
EXECUTE FUNCTION transport_consistency(1);

CREATE OR REPLACE TRIGGER tanks_update_trigger
    BEFORE UPDATE
    ON tanks
    FOR EACH ROW
EXECUTE FUNCTION transport_consistency(2);

CREATE OR REPLACE TRIGGER tractors_update_trigger
    BEFORE UPDATE
    ON tractors
    FOR EACH ROW
