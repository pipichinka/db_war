...
#include <QDateEdit>
CorporalsWindow::CorporalsWindow(QSqlDatabase* db, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::CorporalsWindow),
    model(new QSqlRelationalTableModel(this, *db))
{
    ui->setupUi(this);
    model->setTable("corporals_view");
...
CorporalsWindow::~CorporalsWindow()
{
    delete ui;
}

void CorporalsWindow::on_buttonAdd_clicked()
{
    ...
    q.prepare("INSERT INTO corporals_view (name, work, head, height) "
...
void CorporalsWindow::on_buttonDelete_clicked()
{
    ...
    for (int i = 0; i < list_to_delete.size(); i++){
        ...
        QSqlQuery q(model->database());
        q.prepare("DELETE FROM corporals_view WHERE id = :id");
        q.bindValue(":id", id);
        bool v = q.exec();
        if (v){
            continue;
