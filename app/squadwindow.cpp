...
#include <QDateEdit>
SquadWindow::SquadWindow(QSqlDatabase* db, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::SquadWindow),
    model(new QSqlRelationalTableModel(this, *db))
{
    ui->setupUi(this);
    model->setTable("squad_view");
    model->setRelation(3, QSqlRelation("unit_constructions", "id", "name"));
    model->setRelation(2, QSqlRelation("platoon_view", "id", "name"));
...
void SquadWindow::on_buttonAdd_clicked()
{
    ...
    q.prepare("INSERT INTO squad_view (name, part_of, construction_id) "
...
void SquadWindow::on_buttonDelete_clicked()
{
    ...
    for (int i = 0; i < list_to_delete.size(); i++){
        ...
        QSqlQuery q(model->database());
        q.prepare("DELETE FROM squad_view WHERE id = :id");
        q.bindValue(":id", id);
        bool v = q.exec();
        if (v){
            continue;
