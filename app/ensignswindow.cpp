...
#include <QDateEdit>
EnsignsWindow::EnsignsWindow(QSqlDatabase* db, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::EnsignsWindow),
    model(new QSqlRelationalTableModel(this, *db))
{
    ui->setupUi(this);
    model->setTable("ensigns_view");
...
EnsignsWindow::~EnsignsWindow()
{
    delete ui;
}

void EnsignsWindow::on_buttonAdd_clicked()
{
    ...
    q.prepare("INSERT INTO ensigns_view (name, work, head, hire_date) "
...
void EnsignsWindow::on_buttonDelete_clicked()
{
    ...
    for (int i = 0; i < list_to_delete.size(); i++){
        ...
        QSqlQuery q(model->database());
        q.prepare("DELETE FROM ensigns_view WHERE id = :id");
        q.bindValue(":id", id);
        bool v = q.exec();
        if (v){
            continue;
