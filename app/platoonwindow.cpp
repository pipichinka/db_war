...
#include <QDateEdit>
PlatoonWindow::PlatoonWindow(QSqlDatabase* db, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::PlatoonWindow),
    model(new QSqlRelationalTableModel(this, *db))
{
    ui->setupUi(this);
    model->setTable("platoon_view");
    model->setRelation(3, QSqlRelation("unit_constructions", "id", "name"));
    model->setRelation(2, QSqlRelation("company_view", "id", "name"));
...
PlatoonWindow::~PlatoonWindow()
{
    delete ui;
}

void PlatoonWindow::on_buttonADD_clicked()
{
    ...
    q.prepare("INSERT INTO platoon_view (name, part_of, construction_id) "
...
void PlatoonWindow::on_buttonDelete_clicked()
{
    ...
    for (int i = 0; i < list_to_delete.size(); i++){
        ...
        QSqlQuery q(model->database());
        q.prepare("DELETE FROM platoon_view WHERE id = :id");
        q.bindValue(":id", id);
        bool v = q.exec();
        if (v){
            continue;
