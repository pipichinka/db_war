...
#include <QDateEdit>
CorpWindow::CorpWindow(QSqlDatabase* db, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::CorpWindow),
    model(new QSqlRelationalTableModel(this, *db))
{
    ui->setupUi(this);
    model->setTable("corp_view");
...
CorpWindow::~CorpWindow()
{
    delete ui;
}

void CorpWindow::on_buttonAdd_clicked()
{
    ...
    q.prepare("INSERT INTO corp_view (name, part_of) "
                    "VALUES (:name, :part_of)");
...
void CorpWindow::on_buttonDelete_clicked()
{
    ...
    for (int i = 0; i < list_to_delete.size(); i++){
        ...
        QSqlQuery q(model->database());
        q.prepare("DELETE FROM corp_view WHERE id = :id");
        q.bindValue(":id", id);
        bool v = q.exec();
        if (v){
            continue;
