...
#include "ui_lieutenantswindow.h"
#include <iostream>
...
#include <QDateEdit>
LieutenantsWindow::LieutenantsWindow(QSqlDatabase* db, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::LieutenantsWindow),
    model(new QSqlRelationalTableModel(this, *db))
{
    ui->setupUi(this);
    model->setTable("lieutenants_view");
    ...
    model->setHeaderData(3, Qt::Horizontal, QObject::tr("pet"));
    model->select();
...
LieutenantsWindow::~LieutenantsWindow()
{
    delete ui;
}

void LieutenantsWindow::on_buttonAdd_clicked()
{
    ...
    q.prepare("INSERT INTO lieutenants_view (name, work, head, pet) "
...
void LieutenantsWindow::on_buttonDelete_clicked()
{
    ...
    for (int i = 0; i < list_to_delete.size(); i++){
        ...
        QSqlQuery q(model->database());
        q.prepare("DELETE FROM lieutenants_view WHERE id = :id");
        q.bindValue(":id", id);
        bool v = q.exec();
        if (v){
            continue;
