...
#include <QDateEdit>
CaptainsWindow::CaptainsWindow(QSqlDatabase* db, QWidget *parent) :
    QMainWindow(parent),
    ui(new Ui::CaptainsWindow),
    model(new QSqlRelationalTableModel(this, *db))
{
    ui->setupUi(this);
    model->setTable("captains_view");
...
CaptainsWindow::~CaptainsWindow()
{
    delete ui;
}

void CaptainsWindow::on_buttonAdd_clicked()
{
    ...
    form.push_back(QPair<QString, QWidget*>("hire_date", dateLine));
    ...
    q.prepare("INSERT INTO captains_view (name, work, head, hire_date) "
                    "VALUES (:name, :work, :head, :hire_date)");
    ...
    q.bindValue(":hire_date", dateLine->date());
    bool v = q.exec();
    if (v){
        model->select();
        return;
...
void CaptainsWindow::on_buttonDelete_clicked()
{
    ...
    for (int i = 0; i < list_to_delete.size(); i++){
        ...
        QSqlQuery q(model->database());
        q.prepare("DELETE FROM captains_view WHERE id = :id");
        q.bindValue(":id", id);
        bool v = q.exec();
        if (v){
            continue;
